...
export function explodeShapeLayers() {
  ...
  if (layer.selectedProperties.length == 0) {
    ...
    for (var i = 0; i < layer.selectedProperties.length; i++) {
      ...
      for (var h = 1; h < i; h++) {
        if (
          duplicate.property("ADBE Root Vectors Group").property(h - x).name !=
          layer.selectedProperties[i].name
        ) {
          duplicate
            .property("ADBE Root Vectors Group")
            .property(h - x)
      ...
      for (var k = 1; k <= shapeContents.numProperties - d; k++) {
        if (
          duplicate.property("ADBE Root Vectors Group").property(k - y).name !=
          layer.selectedProperties[i].name
        ) {
          duplicate
            .property("ADBE Root Vectors Group")
